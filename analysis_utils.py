...
def mean_exploration_duration_bar_plotting(idm_id,day_ls=None,sem=False):
    ...
    for file in file_days_ls:
        ...
        data_by_mouse=data_info.groupby("RFID").mean(numeric_only=True)
        data_by_mouse.index.name = None
        data_by_mouse = data_by_mouse.reindex(mouse_ls,fill_value=0)
        ...
        if sem==True:
            sem_by_mouse = data_info.groupby("RFID").sem(numeric_only=True)
...
def feature_table_processor(df_idm):
    ...
    df_idm.fillna({
        "x_loc_distance_duration_high_speed":0,
        "max_speedS": 0,
        "escape_distance": 0,
        "escape_max_acc": 0,
        "fastest_escape_start": 600,
        "latency_to_first_escape": 600,
        "durationsM": 0,
        "sum_duration_lsM": 0,
        "freeze_start": 600,
        "reach_safe_idx": 900
...
def plot_mean_sem_feature_curve(df_idm, vary="num_stim", feature="max_speedS", shelter=0, xrange="default",yrange="default"):
    ...
    mean_value = dc_vary_group.mean(numeric_only=True)
    sem_value = dc_vary_group.sem(numeric_only=True)
